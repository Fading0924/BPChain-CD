...
parser.add_argument('-d', '--dir', type=str, default="./uaidata/uai/", help="relative path to this python source file")
parser.add_argument('-f', '--field', type=float, default=0.1, help="local field value")
parser.add_argument('-i', '--interaction', type=int, default=1, help="0 for attractive, 1 for mixed")
parser.add_argument('-m', '--model', type=str, default="none", help="clique-random clique-structured Ising model; grid-grid Ising model")
...
def getRandomSubset(sz):
    ...
    subset = np.random.choice(range(nbvar), num, replace=False).tolist()
    return subset
...
def JointModel(fp, nbvar):
    with open(fp, 'w+') as f:
        ...
        # 
        # random joint length
        n_constraints = random.randint(nbvar, 2*nbvar)
        f.write(str(n_constraints) + '\n')
        constraints = []
        for i in range(n_constraints):
            var_set = getRandomSubset(nbvar)
        ...
        for i in range(n_constraints):
            ...
            for j in range(tablesize):
                string = string + str(np.random.uniform(0.1, 1, 1)[0] + np.random.choice([0,1], 1)[0] * np.random.uniform(100,1000, 1)[0]) + ' '
            f.write(string + '\n')
            
def SequenceGenerator(fp, nbvar):
    with open(fp, 'w+') as f:
        f.write("MARKOV\n")
        f.write(str(nbvar) + "\n")
        for i in range(nbvar):
        ...
        # 
        # random joint length
        n_constraints = int((nbvar - 5) / 2) + 1
        f.write(str(n_constraints) + '\n')
        constraints = []
        for i in range(n_constraints):
            num = 5
            var_set = [int(j + 2*i) for j in range(5)]
            constraints.append(var_set)
            string = ''
            f.write(str(len(var_set)) + '\n')
            for var in var_set:
        ...
        for i in range(n_constraints):
            ...
            string = ''
            for j in range(tablesize):
                string = string + str(np.abs(np.random.randn()+10)) + ' '
            f.write(string + '\n')

def SequenceGeneratorqq(fp, nbvar):
    with open(fp, 'w+') as f:
        f.write("MARKOV\n")
        f.write(str(nbvar) + "\n")
        for i in range(nbvar):
        ...
        # 
        # random joint length
        n_constraints = int(nbvar / 5)
        f.write(str(n_constraints) + '\n')
        constraints = []
        for i in range(n_constraints):
            num = 5
            var_set = [int(j + 5*i) for j in range(5)]
            constraints.append(var_set)
            string = ''
            f.write(str(len(var_set)) + '\n')
            for var in var_set:
...
if __name__ == "__main__":
    ...
    if args.model == "clique":
        for i in range(4):
            nbvar = 5 * i + 10
            filename = "{}ising_var{:d}.uai".format(args.model, nbvar)
        ...
        for i in range(8):
            nbvar = 3 * i + 10
            filename = "{}ising_var{:d}.uai".format(args.model, nbvar)
            JointModel(datadir+'/'+filename, nbvar)
    elif args.model == 'seq':
        for i in range(0, 14):
            nbvar = 2 * i + 5
            filename = "{}var_{:d}.uai".format(args.model, nbvar)
            SequenceGenerator('./seqdata/seq/' + filename, nbvar)
    elif args.model == 'seqq':
        for i in range(1, 5):
            nbvar = 5 * i
            filename = "{}var_{:d}.uai".format(args.model, nbvar)
            SequenceGeneratorqq('./seqqdata/seqq/' + filename, nbvar)
    else:
        print('Please specify model by --model')
